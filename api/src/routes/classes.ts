...
// Get class roster
router.get("/:id/roster", async (req: AuthRequest, res) => {
  try {
...
router.get("/:id/metrics", async (req: AuthRequest, res) => {
  try {
    ...
    const metrics = students.map((student) => {
      ...
      const avgAccuracyPct =
        student.practiceSessions.length > 0
          ? student.practiceSessions.reduce(
              (sum, session) => sum + session.accuracyPct,
              0
    ...
    const activeStudents = metrics.filter(
      (m) => m.sessionsThisWeek >= 2
    ).length;
    const lowMoodStudents = metrics.filter(
      (m) => m.recentMood !== null && m.recentMood <= 2
    ).length;
    const avgAccuracy =
      metrics.length > 0
        ? Math.round(
            (metrics.reduce((sum, m) => sum + m.avgAccuracyPct, 0) /
              metrics.length) *
              100
          ) / 100
        : 0;
    ...
    const dueAssignments = await prisma.assignment.count({
      where: {
        classId,
        dueAt: {
          gte: new Date(),
